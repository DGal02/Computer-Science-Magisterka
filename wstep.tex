...
Motywacją do podjęcia niniejszego tematu badawczego było rosnące znaczenie alternatywnych technologii przechowywania danych, które wykraczają poza tradycyjny, relacyjny model oparty na języku SQL. Dynamiczny rozwój systemów rozproszonych oraz zapotrzebowanie na mikrosekundowe opóźnienia w dostępie do informacji wymusiły ewolucję w kierunku wyspecjalizowanych baz danych NoSQL.

Dodatkowym czynnikiem skłaniającym do przeprowadzenia badań była potrzeba poszerzenia wiedzy i szczegółowej analizy rozwiązań wykraczających poza system Redis, stanowiący obecnie najpowszechniejszy standard rynkowy w klasie magazynów klucz-wartość. Zestawienie go z silnikami o zróżnicowanej architekturze (takimi jak transakcyjna baza BerkeleyDB, zoptymalizowany pod kątem LuaJIT Tarantool czy rozproszona baza FoundationDB) pozwala na zdefiniowanie obiektywnych kryteriów doboru silnika bazy danych w zależności od specyfiki projektowanego systemu informatycznego.
...
\begin{itemize}
    ...
    \item \textbf{Hipoteza 2:} Bazy osadzone (LevelDB, RocksDB, BerkeleyDB) wykażą wyższą wydajność zapisu w porównaniu do jednowątkowych systemów klient-serwer z uwagi na brak narzutu protokołu sieciowego.
