...
\begin{itemize}
    ...
    \item \textbf{Wsparcie dla transakcji ACID (0 / 100 / 200 pkt):} Ocenia poziom gwarancji spójności danych przy jednoczesnych modyfikacjach. Ocena 200 punktów oznacza pełne wsparcie dla transakcji wielokluczowych (w tym rozproszonych) z zachowaniem pełnej izolacji. Ocena 100 punktów oznacza uproszczoną transakcyjność (np. atomowość na poziomie pojedynczego klucza lub grupowanie operacji bez pełnego wycofywania zmian). Ocena 0 punktów oznacza brak mechanizmów transakcyjnych.
    \item \textbf{Dodatkowe funkcjonalności i unikalność (0 / 150 / 300 pkt):} Ocenia dostępność zaawansowanych mechanizmów wykraczających poza proste operacje Key-Value. Ocena 300 punktów oznacza wielomodelowość i bogate API (np. strumienie, wyszukiwanie pełnotekstowe, wbudowane silniki skryptowe). Ocena 150 punktów oznacza średni stopień rozszerzalności (np. Column Families, różne fizyczne metody dostępu). Ocena 0 punktów oznacza czysty silnik Key-Value bez dodatkowych struktur.
...
Dodatkowo, dla zaawansowanych scenariuszy testowych (kolejka FIFO oraz 6 operacji na dokumentach JSON) wprowadza się punkty za realizację funkcjonalności:
\begin{itemize}
    \item \textbf{10 pkt:} pomyślne ukończenie testu z użyciem natywnego wsparcia dla danej struktury danych po stronie serwera bazy danych (np. Redis, Tarantool, Couchbase).
    \item \textbf{5 pkt:} pomyślne ukończenie testu z wykorzystaniem programowej emulacji struktur po stronie aplikacji klienckiej w języku Python (np. LevelDB, RocksDB, BerkeleyDB, Memcached, FoundationDB).
...
Tabela \ref{tab:punktacja_statyczna} przedstawia bazową punktację architektoniczno-funkcjonalną przed uwzględnieniem wyników wydajnościowych (suma maks. 1200 punktów).
...
FoundationDB & 100 & 500 & 200 & 300 & 100 & 1200 \\ \hline
Tarantool & 100 & 500 & 200 & 300 & 50 & 1150 \\ \hline
Redis & 50 & 500 & 200 & 300 & 50 & 1100 \\ \hline
Couchbase & 50 & 500 & 100 & 300 & 50 & 1000 \\ \hline
BerkeleyDB & 50 & 250 & 200 & 150 & 100 & 750 \\ \hline
RocksDB & 100 & 0 & 100 & 150 & 100 & 450 \\ \hline
Memcached & 100 & 250 & 0 & 0 & 0 & 350 \\ \hline
LevelDB & 100 & 0 & 100 & 0 & 100 & 300 \\ \hline
\end{tabular}%
}
...
\begin{itemize}
    ...
    \item \textbf{Wsparcie dla transakcji ACID (200 pkt):} Mimo braku klasycznego mechanizmu wycofywania zmian (rollback), jednowątkowy model przetwarzania poleceń w połączeniu ze skryptami Lua zapewnia pełną atomowość i najwyższy stopień izolacji (Serializable) wykonywania transakcji w pamięci RAM.
    \item \textbf{Dodatkowe funkcjonalności i unikalność (300 pkt):} Oferowany jest wyjątkowo bogaty zestaw wbudowanych struktur (m.in. listy, zbiory, strumienie, moduły RedisJSON i RediSearch).
...
\begin{itemize}
    ...
    \item \textbf{Wsparcie dla transakcji ACID (100 pkt):} Zapewniona atomowość zapisu grupy operacji przy użyciu mechanizmu WriteBatch i dziennika WAL.
...
\begin{itemize}
    ...
    \item \textbf{Wsparcie dla transakcji ACID (100 pkt):} Atomowość zapisu realizowana przez mechanizm WriteBatch i dziennik WAL.
    \item \textbf{Dodatkowe funkcjonalności i unikalność (150 pkt):} Rozbudowane mechanizmy optymalizacji (Column Families, Merge Operators, filtry Blooma).
    \item \textbf{Trwałość danych (100 pkt):} Domyślny, silny zapis sekwencyjny do logu WAL na dysk.
\end{itemize}

\paragraph{Couchbase (1000 pkt)}
\begin{itemize}
    ...
    \item \textbf{Wsparcie dla transakcji ACID (100 pkt):} Zapewniono atomowość na poziomie pojedynczego dokumentu JSON.
    \item \textbf{Dodatkowe funkcjonalności i unikalność (300 pkt):} Silnik zapytań SQL++ (N1QL), wbudowane usługi wyszukiwania pełnotekstowego (FTS), analityki oraz zdarzeń (Eventing).
...
\begin{itemize}
    ...
    \item \textbf{Wsparcie dla transakcji ACID (200 pkt):} Pełne transakcje ACID dla silnika Memtx dzięki jednowątkowej architekturze opartej na kooperatywnych włóknach (fibers).
    \item \textbf{Dodatkowe funkcjonalności i unikalność (300 pkt):} Pełnoprawny serwer aplikacji umożliwiający uruchamianie kodu Lua bezpośrednio w procesie bazy danych.
    \item \textbf{Trwałość danych (50 pkt):} Asynchroniczny lub okresowy zapis dziennika transakcji WAL i zrzut migawek na dysk.
\end{itemize}

\paragraph{FoundationDB (1200 pkt)}
\begin{itemize}
    ...
    \item \textbf{Wsparcie dla transakcji ACID (200 pkt):} Globalne transakcje rozproszone o najwyższym poziomie izolacji (Serializable).
    \item \textbf{Dodatkowe funkcjonalności i unikalność (300 pkt):} Architektura oparta na warstwach (Layers), umożliwiająca emulację dokumentów czy relacji grafowych na bazie podstawowego silnika klucz-wartość.
...
\begin{itemize}
    ...
    \item \textbf{Wsparcie dla transakcji ACID (200 pkt):} Pełne lokalne transakcje ACID z menedżerem blokad na poziomie stron i rekordów.
    \item \textbf{Dodatkowe funkcjonalności i unikalność (150 pkt):} Obsługa czterech różnych struktur fizycznych przechowywania danych (B-Tree, Hash, Queue, Recno).
