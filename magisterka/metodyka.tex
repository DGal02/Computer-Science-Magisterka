...
\begin{itemize}
    \item \textbf{LevelDB:} stosuje mechanizm wyłącznej blokady zapisu na poziomie plików bazy danych. Uruchomienie drugiego procesu klienckiego próbującego uzyskać dostęp do tej samej bazy skutkuje natychmiastowym błędem, co uniemożliwiłoby współbieżny dostęp wieloprocesowy.
    \item \textbf{RocksDB i BerkeleyDB:} udostępniają zaawansowane mechanizmy synchronizacji i współdzielenia struktur danych, co pozwala na dostęp wielu procesom (w przypadku BerkeleyDB poprzez podsystem współdzielonej pamięci środowiska). Eksperymenty mają na celu zmierzenie narzutu na synchronizację międzyprocesową w porównaniu do komunikacji sieciowej baz serwerowych.
\end{itemize}
